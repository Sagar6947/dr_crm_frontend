        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Manage global branches and clinical operations.
            </p>
          </div>
          {/* <Link
...
>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-50">
                  <th className="px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                    Clinic Name
                  </th>
                  <th className="px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                    Location
              ...
              <tbody className="divide-y divide-slate-50">
                {isLoading ? (
                  ...
                  <tr>
                    <td
                      colSpan={6}
                      className="px-8 py-10 text-center text-slate-400 font-medium text-sm"
                    >
                      No clinics found.
                    </td>
                  </tr>
                  ...
                  filteredClinics.map((clinic) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-6">
                        ...
                        >
                          <div
                            className={`w-1.5 h-1.5 rounded-full mr-2 ${clinic.status === "Active" ? "bg-emerald-500" : "bg-slate-400"}`}
                          />
                          {clinic.status}
                        </span>
                      </td>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-2">
                          <Link
                            ...
                            onClick={() => setActionLoading(`view-${clinic.id}`)}
  ...
  <Eye className="w-3 h-3 group-hover/btn:scale-110 transition-transform" />
                          ...
                          <Link
                            ...
                            onClick={() => setActionLoading(`edit-${clinic.id}`)}
  ...
  <Loader2 className="w-3 h-3 animate-spin" />
                          ...
                          <Link
                            ...
                            onClick={() => setActionLoading(`manage-${clinic.id}`)}
                            className="inline-flex items-center gap-2 px-4 py-2 border border-slate-100 rounded-xl text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:border-medical-teal hover:text-medical-teal hover:bg-teal-50/30 transition-all group/btn"
                          >
                            Manage{" "}
                            {/* <ExternalLink className="w-3 h-3 group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5 transition-transform" /> */}
                            {actionLoading === `manage-${clinic.id}` ? (
  <Loader2 className="w-3 h-3 animate-spin" />
) : (
  <ExternalLink className="w-3 h-3 group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5 transition-transform" />
)}
          ...
          <div className="px-8 py-6 bg-slate-50/50 flex justify-between items-center">
            <p className="text-xs font-bold text-slate-400">
              Showing {clinics.length} of {pagination.total_records} clinical
              nodes (Page {pagination.current_page} of {pagination.total_pages})
            </p>
